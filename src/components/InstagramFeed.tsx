...
function InstaCard({ post, index }: { post: InstaPost; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-brand-navy/30 opacity-0 group-hover:opacity-100 transition-opacity duration-[var(--duration-hover)] flex items-center justify-center space-x-8 z-20 pointer-events-none hidden md:flex">
      ...
      {/* Footer */}
      <div className="p-4 md:p-6 border-t border-brand-blue-text/10 bg-white/30 backdrop-blur-md relative z-10 w-full flex flex-col space-y-0.5 md:space-y-1">
        <p className="font-semibold text-brand-slate line-clamp-1 text-[9px] md:text-[11px]">
          {post.description}
        </p>
        <span className="font-black tracking-widest text-brand-blue-text uppercase text-[7px] md:text-[9px]">
          {post.type}
        </span>
      </div>
    </motion.a>
